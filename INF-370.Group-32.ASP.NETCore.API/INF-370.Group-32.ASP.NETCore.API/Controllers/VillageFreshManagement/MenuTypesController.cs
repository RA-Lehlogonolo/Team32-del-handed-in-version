...
namespace INF_370.Group_32.ASP.NETCore.API.Controllers.VillageFreshManagement
{
    ...
    public class MenuTypesController : ControllerBase
    {
        ...
        public MenuTypesController(AppDbContext context)
        {
            _context = context;
        }
        ...
        {
            var recordInDb = _context.MenuTypes.Where(item => item.Id == id).Select(item => new GetGenericNameAndIdDto()
            {
                Name = item.Name,
                Id = item.Id
            }).First();
        ...
        {
            var recordsInDb = _context.MenuTypes.Select(item => new GetGenericNameAndIdDto
            {
                Name = item.Name,
                Id = item.Id
        ...
        public IActionResult Update(AddOrUpdateGenericNameOnlyDto model, string username, int id)
        {
            ...
            {
                var recordInDb = _context.MenuTypes.FirstOrDefault(item => item.Id == id);
        ...
        public IActionResult Add(AddOrUpdateGenericNameOnlyDto model, string username)
        {
            ...
            if (ModelState.IsValid)
            {
                var recordInDb = _context.MenuTypes.FirstOrDefault(item => item.Name.ToLower() == model.Name.ToLower());
                ...
                var newRecord = new MenuType()
                {
                    Name = model.Name
                };
                _context.MenuTypes.Add(newRecord);
                _context.SaveChanges();
                return Ok();
            }
        ...
        public async Task<ActionResult<MenuType>> Delete(int id)
        {
            var recordInDb = await _context.MenuTypes.FindAsync(id);
            if (recordInDb == null)
            {
                return NotFound();
            }
            ...
            _context.MenuTypes.Remove(recordInDb);
