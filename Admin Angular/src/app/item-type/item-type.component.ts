 ...
 //Add Operations
  ...
  onAddSubmit(): void {
    if (this.addForm.valid) {
      ...
      this._VillageFreshService.addItemType(this.addForm.value, this.loggedInUser.UserName)
        .subscribe(event => {
          ...
          if (event.type === HttpEventType.Response) {
            ...
            this._nzNotificationService.success("Add Success", "New Item Type Added");
 ...
 //Update Operations
  ...
  private buildUpdateForm(role: GenericNameAndIdType) {
   ...
   onUpdateSubmit(): void {
    if (this.updateForm.valid) {
      ...
      this._VillageFreshService.updateItemType(this.updateForm.value, this.loggedInUser.UserName, updateId)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
          if (event.type === HttpEventType.Response) {
            ...
            this.onCancelUpdate();
            this.tableDataReady = true;
            this._nzNotificationService.success("Update Success", "Item Type Updated");
          }
        },
          error => {
            this.tableDataReady = true;
            this._nzNotificationService.error('Update Item Type Error', error.error.message);
          })
    }
  }
  ...
  onDeleteSubmit(): void {
    ...
    if (this.itemToDelete != null) {
      this._VillageFreshService.deleteItemType(this.loggedInUser.UserName, this.itemToDelete.id)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
