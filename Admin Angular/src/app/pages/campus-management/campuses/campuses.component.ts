...
export class CampusesComponent implements OnInit {
  ...
  onAddSubmit(): void {
    if (this.addForm.valid) {
      ...
      this._campusManagementService.addCampus(this.addForm.value, this.loggedInUser.UserName)
        .subscribe(event => {
  ...
  onUpdateSubmit(): void {
    if (this.updateForm.valid) {
      ...
      this._campusManagementService.updateCampus(this.updateForm.value, this.loggedInUser.UserName, updateId)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
          if (event.type === HttpEventType.Response) {
            ...
            this.onCancelUpdate();
            this.tableDataReady = true;
            this._nzNotificationService.success("Update Success", "Campus Updated");
          }
        },
          error => {
            this.tableDataReady = true;
            this._nzNotificationService.error('Update Campus Error', error.error.message);
          })
    }
  }
  ...
  onDeleteSubmit(): void {
    ...
    if (this.itemToDelete != null) {
      this._campusManagementService.deleteCampus(this.loggedInUser.UserName, this.itemToDelete.id)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
