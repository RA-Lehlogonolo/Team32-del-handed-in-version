...
import { HouseParent } from 'src/app/services/users-management/users-management.types';

@Component({
  selector: 'app-manage-house-parents',
  templateUrl: './manage-house-parents.component.html',
  styleUrls: ['./manage-house-parents.component.css']
})
export class ManageHouseParentsComponent implements OnInit {
  addForm: FormGroup;
  ...
  listOfData: HouseParent[] = [];
  listOfDisplayData: HouseParent[] = [];
  ...
  search(): void {
    this.visible = false;
    this.listOfDisplayData = this.listOfData.filter((item: HouseParent) => item.username.indexOf(this.searchValue) !== -1);
  }
  ...
  onAddSubmit(): void {
    if (this.addForm.valid) {
      ...
      this._userManagementService.addHouseParent(this.addForm.value, this.loggedInUser.UserName)
        .subscribe(event => {
  ...
  private getAllRecords() {
    this._userManagementService.getAllHouseParents()
      .subscribe(event => {
        if (event.type === HttpEventType.Sent) {
          this.tableDataReady = false;
        }
        if (event.type === HttpEventType.Response) {
          ...
          this.listOfData = event.body as HouseParent[];
  ...
  onResendAccountCreatedNotification(element:HouseParent){
