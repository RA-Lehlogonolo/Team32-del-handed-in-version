...
export class AnnouncementTypeComponent implements OnInit {
  ...
  onAddSubmit(): void {
    if (this.addForm.valid) {
      ...
      this._announcementService.addAnnouncementType(this.addForm.value, this.loggedInUser.UserName)
        .subscribe(event => {
          ...
          if (event.type === HttpEventType.Response) {
            ...
            this._nzNotificationService.success('Add Success', 'New Announcement Type Added');
  ...
  private buildUpdateForm(role: GenericNameAndIdType) {
   ...
   onUpdateSubmit(): void {
    if (this.updateForm.valid) {
      ...
      this._announcementService.updateAnnouncementType(this.updateForm.value, this.loggedInUser.UserName, updateId)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
          if (event.type === HttpEventType.Response) {
            ...
            this.onCancelUpdate();
            this.tableDataReady = true;
            this._nzNotificationService.success('Update Success', 'Announcement Type Updated');
          }
        },
          error => {
            this.tableDataReady = true;
            this._nzNotificationService.error('Update Announcement Type Error', error.error.message);
          });
    }
  }
  ...
  onDeleteSubmit(): void {
    ...
    if (this.itemToDelete != null) {
      this._announcementService.deleteAnnouncementType(this.loggedInUser.UserName, this.itemToDelete.id)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
