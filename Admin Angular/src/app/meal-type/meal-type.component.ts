 ...
 addForm: FormGroup;
  ...
  onAddSubmit(): void {
    if (this.addForm.valid) {
      ...
      this._VillageFreshService.addMealType(this.addForm.value, this.loggedInUser.UserName)
        .subscribe(event => {
          ...
          if (event.type === HttpEventType.Response) {
            ...
            this._nzNotificationService.success("Add Success", "New Meal Type Added");
  ...
  private buildUpdateForm(role: GenericNameAndIdType) {
   ...
   onUpdateSubmit(): void {
    if (this.updateForm.valid) {
      ...
      this._VillageFreshService.updateMealType(this.updateForm.value, this.loggedInUser.UserName, updateId)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
          if (event.type === HttpEventType.Response) {
            ...
            this.onCancelUpdate();
            this.tableDataReady = true;
            this._nzNotificationService.success("Update Success", "Meal Type Updated");
          }
        },
          error => {
            this.tableDataReady = true;
            this._nzNotificationService.error('Update Meal Type Error', error.error.message);
          })
    }
  }
  ...
  onShowDeleteModal(item: GenericNameAndIdType): void {
    ...
    if (this.itemToDelete != null) {
      this._VillageFreshService.deleteMealType(this.loggedInUser.UserName, this.itemToDelete.id)
        .subscribe(event => {
          if (event.type === HttpEventType.Sent) {
            this.tableDataReady = false;
          }
...
//Get & Helper Operations
  private getAllRecords() {
    this._VillageFreshService.getAllMealTypes()
      .subscribe(event => {
        if (event.type === HttpEventType.Sent) {
          this.tableDataReady = false;
        }
